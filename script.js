...
// ------------------ TRANSFERIR CONTA (popup) ------------------
const popupTransferir = document.getElementById("popup-transferir");
document.getElementById("transferir-conta-btn").addEventListener("click", () => {
  popupTransferir.classList.add("active");
});
...
document.getElementById("confirmar-transferencia").addEventListener("click", () => {
  ...
  if (origem && destino && pedidosPorMesa[origem]) {
    pedidosPorMesa[destino] = (pedidosPorMesa[destino] || []).concat(pedidosPorMesa[origem]);
    pedidosPorMesa[origem] = [];
    salvarLocalStorage();
    alert(`Conta da Mesa ${origem} transferida para a Mesa ${destino}.`);
    popupTransferir.classList.remove("active");
  } else {
    alert("Mesas inválidas ou sem pedidos.");
  }
});

// ------------------ JUNTAR MESAS (popup) ------------------
const popupJuntar = document.getElementById("popup-juntar");
document.getElementById("juntar-mesas-btn").addEventListener("click", () => {
  popupJuntar.classList.add("active");
});

document.getElementById("confirmar-juncao").addEventListener("click", () => {
  const mesa1 = document.getElementById("juntar-mesa1").value;
  const mesa2 = document.getElementById("juntar-mesa2").value;

  if (mesa1 && mesa2 && pedidosPorMesa[mesa1] && pedidosPorMesa[mesa2]) {
    pedidosPorMesa[mesa1] = pedidosPorMesa[mesa1].concat(pedidosPorMesa[mesa2]);
    pedidosPorMesa[mesa2] = [];
    salvarLocalStorage();
    alert(`Mesas ${mesa1} e ${mesa2} foram unidas na Mesa ${mesa1}.`);
    popupJuntar.classList.remove("active");
  } else {
    alert("Mesas inválidas ou sem pedidos.");
  }
});
